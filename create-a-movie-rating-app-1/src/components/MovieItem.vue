...
const props = defineProps(["movie", "handleDelete", "handleEdit"]);

const handleStar = (n) => {
  props.movie.rating = n;
};
</script>

<template>
  <div class="relative flex h-[32rem] flex-col items-start justify-between rounded overflow-hidden bg-white">
    ...
    <section class="px-5 flex flex-col justify-between pb-2 h-full w-full">
      ...
      <p class="text-sm mt-4 flex items-center gap-2 flex-row">
        ...
        <span class="flex flex-row items-center">
          <button v-for="n in 5" @click="() => handleStar(n)" :disabled="n === movie.rating">
